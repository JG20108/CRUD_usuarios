...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const versionData = await user.getUsuarioVersion();
...
router.post('/new', async (req, res) => {
  try {
    ...
    const newUsuario = await user.addUsuario({email,
        nombre,
        avatar,
        password,
        estado});
...
router.put('/update/:codigo', async (req, res)=>{
  try {
    ...
    const updateResult = await user.updateUsuario({codigo:parseInt(codigo), email, nombre, avatar, password, estado });
    if (!updateResult) {
